...
class FileHelper {
  ...
  writeToFile = async (filepath, data, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      if (o.nextFileName && fs.existsSync(filepath)) {
        ...
        this._promisedWriteToFile(dir + file, data, o, resolve, reject);
        ...
        if (fileManager.tryLock(filepath)) {
          this._promisedWriteToFile(filepath, data, o, resolve, reject);
        } else {
          fileManager.queue(filepath, data, [o, resolve, reject], this._promisedWriteToFile, true);
        }
      } else {
        logger.report(
          this,
  ...
  appendToFile = async (filepath, data, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      if (fileManager.tryLock(filepath)) {
        this._promisedAppendToFile(filepath, data, o, resolve, reject);
      }
      fileManager.queue(filepath, data, [o, resolve, reject], this._promisedAppendToFile, true);
    });
  };
  ...
  /**
   * Refactoring this to private functions located outside of this helper
   */
