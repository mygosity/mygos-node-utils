...
class FileHelper {
  ...
  promisedWriteToFile = (filepath, data, o, resolve, reject) => {
    fs.writeFile(filepath, o.jsonStringify ? JSON.stringify(data) : data, function(error) {
      ...
      if (error) {
        logger.error(error, { src: 'FileHelper|nextFileName', funcName: 'writeToFile' });
        reject(error);
      }
...
const _writeJsonFile = function(filepath, jsondata, options, resolve, reject) {
  const _createJson = function() {
    fs.writeFile(filepath, options.jsonWrapper, function(error) {
      if (error) {
        logger.error(error, { src: 'FileHelper_Priv', funcName: '_writeJsonFile|_createJson|writeFile', filepath, jsondata, options });
  ...
  const onFail = (error, size) => {
    if (error != undefined && size !== 0) {
      fileManager.release(filepath);
      logger.error(error, { src: 'FileHelper_Priv', funcName: 'onFail', size, filepath, data, options });
    } else {
      _createJson();
    }
  };
...
const _appendToJsonFile = function(filepath, data, onFail, resolve, reject) {
  fs.stat(filepath, function(errorStat, stat) {
    if (errorStat || stat.size === 0) {
      if (errorStat) logger.error(errorStat, { src: 'FileHelper_Priv', funcName: '_appendToJsonFile|stat', filepath, data, stat });
    ...
    stream.on('error', function(streamError) {
      ...
      logger.error(streamError, { src: 'FileHelper_Priv', funcName: '_appendToJsonFile|on("error"', filepath, data, stat, streamArgs, size });
      onFail(streamError);
