  ...
  writeToFile = async (filepath, data, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      if (o.nextFileName && fs.existsSync(filepath)) {
        ...
        logger.report(this, 'writeToFile:: aborted writing due to options set: ' + filepath, options);
...
function _getNextFileName(file, fileMap, dir, options) {
  ...
  while (fileMap[nextFileName] && _isSizeExceeded(dir + nextFileName, options)) {
    numbers++;
...
function _autoParseNextFileName(filepath, options) {
  const [dir, file] = options.relativePath ? _splitFolderAndFile(fileHelper.getRelativePath(filepath)) : _splitFolderAndFile(filepath);
  logger.report(fileHelper, '_autoParseNextFileName:: dir, file: ', dir, file);
  const files = fs.readdirSync(dir);
  const fileMap = {};
  files.map((i) => (fileMap[i] = true));
  const newFileName = _getNextFileName(file, fileMap, dir, options);
  logger.report(fileHelper, '_autoParseNextFileName::', dir + newFileName);
