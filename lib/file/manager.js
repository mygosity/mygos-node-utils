...
class FileManager {
  ...
  tryLock = (filepath) => {
    if (filelocks[filepath] === true) {
      logger.report(this, 'lock: file is already locked:' + filepath);
      return false;
    }
  ...
  __status = () => {
    ...
    logger.writelog(this, { settings, src: copy(this), filelocks: copy(filelocks), writequeue: copy(writequeue) });
  };
}
