...
class FileManager {
  queue = (filepath, jsondata, options, writeCallback) => {
    // TODO:: these will change on verbosity options for logging in future
  ...
  releaseQueue(filepath) {
    ...
    for (let i = 0; i < target.length; ++i) {
      if (target[i] != null) {
        ...
        if (copiedOptions === undefined)
          copiedOptions = { ...target[i].options };
        target[i] = null;
      }
    }
    if (allNull) {
  ...
  __status = () => {
    ...
    logger.writelog({
      signature,
      src: copy(this),
      filelocks: copy(filelocks),
      writequeue: copy(writequeue)
    });
  };
}
