...
function LogService() {
  ...
  this.logban = (src, ...args) => {
    try {
      const data = { timestamp: dateUtils.getAusTimestamp(this.creationTime), src, args };
      const folder = dateUtils.getDateFormat(this.creationTime);
      const folderPath = `${this.loggingPath}${folder}`;
      const filePath = `${folderPath}/logban.json`;
      this.report(src, { func: `LogService=>logban::` }, filePath);
