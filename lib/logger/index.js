...
function LogService() {
  ...
  this.writelog = (src, ...args) => {
    try {
      ...
      this.report({ src: src && src.logSignature ? src.logSignature : "unknown=>", func: `LogService=>writelog::` }, filePath);
  ...
  this.error = (src, error, ...args) => {
    try {
      const errorMsg = error ? (error.toString ? error.toString() : error) : "unknown::";
      ...
      this.report({ src: src && src.logSignature ? src.logSignature : "unknown=>", func: `LogService=>error::` }, filePath);
      fileHelper.assertDirExists(folderPath);
      fileHelper.writeContinuousJson(filePath, data);
    } catch (error) {
      this.outputMethod(error);
