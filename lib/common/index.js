...
export function isObject(target) {
  ...
  return type === "object" && !Array.isArray(target) && type !== "function";
}
...
export function deepCloneObject(target, dupeChecker = []) {
  ...
  for (let type in target) {
    if (typeof target[type] !== "object") {
      ...
      if (!isClonedIn(target[type], dupeChecker)) {
        ...
        const copy = deepCloneObject(target[type], dupeChecker);
        if (copy !== undefined) {
          clone[type] = copy;
        }
      } else {
